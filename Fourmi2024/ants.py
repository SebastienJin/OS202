...
class Colony:
    ...
    """
    def __init__(self, start_idx, end_idx, pos_init, max_life):
        # Each ant has is own unique random seed
        self.seeds = np.arange(start_idx+1, end_idx+1, dtype=np.int64)
        self.nb_ants = len(self.seeds)
        # self.nb_ants = end_idx - start_idx
        # self.seeds = np.arange(1, self.nb_ants+1, dtype=np.int64)
        # State of each ant : loaded or unloaded
        self.is_loaded = np.zeros(self.nb_ants, dtype=np.int8)
        ...
        self.max_life = max_life * np.ones(self.nb_ants, dtype=np.int32)
        self.max_life -= np.int32(max_life*(self.seeds/147483647.))//4
        # Ages of ants : zero at beginning
        self.age = np.zeros(self.nb_ants, dtype=np.int64)
        # History of the path taken by each ant. The position at the ant's age represents its current position.
        self.historic_path = np.zeros((self.nb_ants, max_life+1, 2), dtype=np.int16)
        ...
        self.directions = d.DIR_NONE*np.ones(self.nb_ants, dtype=np.int8)
    ...
    def merge(self, other_colony):
        """
        Merge the data from another colony into this one.

        Parameters:
        other_colony (Colony): The other colony instance to merge data from.
        """
        # 确保合并的另一个Colony实例是Colony类型
        if not isinstance(other_colony, Colony):
            raise ValueError("other_colony must be an instance of Colony")

        # 合并各种属性
        self.seeds = np.concatenate((self.seeds, other_colony.seeds))
        self.is_loaded = np.concatenate((self.is_loaded, other_colony.is_loaded))
        self.max_life = np.concatenate((self.max_life, other_colony.max_life))
        self.age = np.concatenate((self.age, other_colony.age))
        self.historic_path = np.concatenate((self.historic_path, other_colony.historic_path), axis=0)
        self.directions = np.concatenate((self.directions, other_colony.directions))
        self.nb_ants = len(self.seeds)
        # Note: 如果sprites图像相同，不需要合并，只要保留一份即可。
        return self

def display(ants, screen):
    sprites = []
    img = pg.image.load("ants.png").convert_alpha()
    for i in range(0, 32, 8):
